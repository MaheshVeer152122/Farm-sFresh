...
public class FarmersDaoImpl implements IFarmersDao {
	...
	public List<StockDetails> getFarmerStock(int farmerid) {
		String jpql = "SELECT NEW com.marketplace.pojos.StockDetails(sd.id, sd.stockItem, sd.pricePerUnit) FROM StockDetails sd JOIN sd.farmer1 f WHERE f.farmerId=:frmr";
